    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (save.ShowDialog() == DialogResult.OK)
            {
                ...
                Kayit.WriteLine(make_h_file2(safeFileAddress));
                Kayit.Close();
        ...
        private string make_h_file2(string filePath)
        {
            ...
            main_msg += "#define LIB_\r\n\r\n";

            main_msg += "#ifdef __cplusplus\r\n";
            main_msg += "extern \"C\" {\r\n";
            main_msg += "#endif\r\n\r\n";
            main_msg += "#include <stdint.h>\r\n";
            main_msg += "#ifndef __cplusplus\r\n";
            main_msg += "#include <stdbool.h>\r\n";
            main_msg += "#endif\r\n\r\n";
            ...
            main_msg += "\n/*______USER CODE FUNCTION BLOCK END______*/\n\n";
            ...
            foreach (var msg in sortedmsgs)
            {
                ...
                foreach (var sig in sortedSignals)
                {
                    if (sig.Factor != 1 || sig.Offset != 0)
                    {
                        ...
                        main_msg += $"\tdbc->{msg.Name}.{sig.Name}.Phys_Value.factor = {sig.Factor.ToString(CultureInfo.InvariantCulture)};\n";
                        main_msg += $"\tdbc->{msg.Name}.{sig.Name}.Phys_Value.offset = {sig.Offset.ToString(CultureInfo.InvariantCulture)};\n";
                    }
                }
            }
            main_msg += "}\n\n";
        ...
        private string make_cpp_file2(string filePath)
        {
            ...
            main_msg += "#ifdef READ_ENABLE\n";
            main_msg += "void ReadParse(uint8_t* rx_data, uint32_t id, DbcStruct *dbc) {\n";
            main_msg += "\tswitch (id) {\n";
            ...
            foreach (var msg in sortedmsgs)
            {
                ...
                main_msg += $"\t\tcase {msg.Name}_ID:\n";
                main_msg += $"\t\t\tfor(int i = 0; i < {msg.DLC}; ++i) {{\n";
                main_msg += $"\t\t\t\tdbc->{msg.Name}.Data[i] = rx_data[i];\n";
                main_msg += "\t\t\t}\n";
                foreach (var sig in sortedSignals)
                {
                    if (sig.Factor != 1 || sig.Offset != 0)
            ...
            main_msg += "\t}\n";
            main_msg += "}\n";
            main_msg += "#endif // READ_ENABLE\n\n";

            foreach (var msg in sortedmsgs)
            {
                main_msg += $"void CreateTable_{msg.Name}(DbcStruct *dbc) {{\n";
                var sortedSignals = msg.Signals.OrderBy(sig => sig.StartBit);
                foreach (var sig in sortedSignals)
                {
                    if ((sig.Factor != 1) || (sig.Offset != 0))
            ...
            main_msg += "uint32_t PHYS_TO_RAW(int phys_value, Phys_Value_t *phys_struct) {\n";
            main_msg += "\treturn static_cast<uint32_t>((phys_value - phys_struct->offset) / phys_struct->factor);\n";
            main_msg += "}\n\n";

            main_msg += "uint32_t RAW_TO_PHYS(int raw_value, Phys_Value_t *phys_struct) {\n";
            main_msg += "\treturn static_cast<uint32_t>((raw_value * phys_struct->factor) + phys_struct->offset);\n";
            main_msg += "}\n";
        ...
        {
            if (filename.EndsWith(".dbc", StringComparison.OrdinalIgnoreCase))
            {
